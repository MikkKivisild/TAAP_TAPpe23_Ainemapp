...
function showNumberedProducts(count, skip = []){
    let finish = count === undefined ? products.length : count;
    let loopsDone =0;
...
function listOfAveragePerMonth(){
    monthNumber=1;
    for (const monthTemps of temperatures){
        const average = getAverage(monthTemps);
        document.write(`<li>${getMonthName(monthNumber++)} keskmine temperatuur ${average}</li>`)
    }
}
function highestLowestTemperature(){
    let highestTemperature = 1, highestTemp;
    let LowestTemperature, lowestTemp;
